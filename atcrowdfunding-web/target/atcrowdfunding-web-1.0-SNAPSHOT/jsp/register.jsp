...
<div class="container">
    ...
    <form id="loginForm" action="doLogin" method="post" class="form-signin" role="form">
        ...
        <div class="form-group has-success has-feedback">
            <input type="text" class="form-control" id="username" name="username" placeholder="请输入姓名" autofocus>
        ...
        <a class="btn btn-lg btn-success btn-block" onclick="btnregister()" > 注册</a>
    </form>
</div>
...
<script>
    ...
    function btnregister() {
        alert("aaaa"+$("#loginacct").val())
        // 非空校验
        var loginacct = $("#loginacct").val();
        // 表单元素的value取值不会为null, 取值是空字符串
        if ( loginacct == "" ) {
            //alert("用户登录账号不能为空，请输入");
            layer.msg("用户注册账号不能为空，请输入", {time:2000, icon:5, shift:6}, function(){
        ...
        if ( userpswd == "" ) {
            ...
            layer.msg("用户注册密码不能为空，请输入", {time:2000, icon:5, shift:6}, function(){
        ...
        console.log(loginacct+"==="+username+"==="+userpswd);
        $.ajax({
            type : "POST",
            url  : "doAJAXRegister",
            data : {
